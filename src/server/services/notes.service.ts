...
import { createNoteSchema, updateNoteSchema } from '@/server/validators/notes.schema';
...
function hydrateNote(note: Prisma.NoteGetPayload<{}>) {
  return {
    ...note,
...
export async function createNote(input: unknown) {
  try {
    ...
    if (error instanceof ZodError) {
      throw new ServiceError(error.errors.map((err) => err.message).join(', '), 400);
    }
    throw error;
  }
}
...
export async function archiveNote(id: string) {
  ...
  const note = await prisma.note.update({ where: { id }, data: { archived: true } });
  return hydrateNote(note);
}
